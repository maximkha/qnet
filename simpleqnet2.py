...
def forward(x, P, lay_N):
    ...
    for i in range(lay_N):
        # print(f"{norm(np.sqrt(x_))=}")
        x_ /= norm(x_)
        x_ = np.abs((x_ @ layer_mat(P, i)))**2
...
print(f"{multi_forward(state_vec, solution.x, N_LAY).real=}")
print(f"{multi_forward(state_vec, solution.x, 1).real=}")
print(f"{multi_forward(state_vec, solution.x, N_LAY).real.sum(axis=1)=}")
print(f"{multi_forward(state_vec, solution.x, 1).real.sum(axis=1)=}")
